...
<head>
    <meta charset="UTF-8">
    <title>Index</title>
    <link rel="stylesheet" href="/resources/user/CSS/qwrite.css" />
   ...
   <%@include file="../user/includes/menu.jsp" %>
    <div id="content">
        ...
        <div id="menu1">
            <nav>
                <ul>
                   <li id="info3" onclick="location.href='/user/product1?pcode=${view.pcode}'">상품정보</li>
            <li id="review3" onclick="location.href='/user/product2?pcode=${view.pcode}'">리뷰</li>
            <li id="QandA3" onclick="location.href='/user/product3?pcode=${view.pcode}'">Q&A</li>
                </ul>

            </nav>
        </div>
        <div id="s_list4">
        <form action="/user/qinsertOk" method="post">
            <table>
				<thead>
					<tr>
						<th>작성자</th>
				...
				<tbody>
					<tr>
                        <td>${read.mid}<input type="hidden" name="scode"value="${view.scode}">
                        <input type="hidden" name="pcode"value="${view.pcode}"></td>
                        <td>${view.pname}</td>
                        <td><textarea name="question" cols="120" rows="8"></textarea></td>
                    </tr>
                 
				</tbody>
				
            </table> 
            <button type="submit" id="qwrite">작성하기</button>
        </form>
        </div>
    </div>
